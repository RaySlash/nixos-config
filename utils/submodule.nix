...
let
  ...
  callModule = module: args:
    let moduleToImport = if args == null then module else import module args;
    in {
      imports = [ moduleToImport ];
    } // lib.optionalAttrs (builtins.any (p: p module) [ isPath isString ]) {
      _file = module;
    };
  ...
  submodule = lib.types.coercedTo from (m: { module = m; }) (lib.types.submodule
    ({ name, config, ... }: {
      ...
      config = { wrappedModule = callModule config.module config.extraArgs; };
    }));
in { inherit callModule submodule; }
